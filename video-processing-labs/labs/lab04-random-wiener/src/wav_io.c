/* copied from lab01-sampling-quantization (adapted for lab04-random-wiener) */
#include "wav_io.h"
#include <math.h>
#include <stdio.h>
#include <stdlib.h>
...
int wav_write(const char *path, const wav_data *w) {
    ...
    fwrite("RIFF",1,4,f); wr_u32(f,36+data_bytes); fwrite("WAVE",1,4,f);
    fwrite("fmt ",1,4,f); wr_u32(f,16); wr_u16(f,1); wr_u16(f,w->channels);
    ...
    fwrite("data",1,4,f); wr_u32(f,data_bytes);
