...
/* Sparse header: n(4B) + nnz(2B) + nnz * (int16 symbol, uint16 freq) */
int huffman_encode(const int *sym, int n, uint8_t *out, int out_cap, int *out_bits) {
    ...
    for (int i = 0; i < nused; i++) {
        ...
        uint16_t fv = (uint16_t)(freq[used[i]] > 65535 ? 65535 : freq[used[i]]);
        memcpy(out + 6 + i * 4, &sv, 2);
        memcpy(out + 8 + i * 4, &fv, 2);
    }
...
int huffman_decode(const uint8_t *in, int in_bytes, int *sym, int max_n) {
    ...
    for (int i = 0; i < (int)nnz; i++) {
        int16_t sv;
        uint16_t fv;
        memcpy(&sv, in + 6 + i * 4, 2);
        memcpy(&fv, in + 8 + i * 4, 2);
