/*
 * main.c —— lab9 驱动：AST → 四元式 →【优化】→ 后端（四选一）
 *
 * 用法: ./minicc9.exe [-O] [选项] 文件.mc
 *   （缺省）生成 x86-64 汇编到 stdout；链接运行：
 *              ./minicc9.exe prog.mc > prog.s
...
int main(int argc, char **argv)
{
    ...
    if (!path) {
        fprintf(stderr,
            "用法: %s [-O] [-ir|-vm|-irvm|-eval|-trace|-ast|-symtab] 文件.mc\n",
            argv[0]);
        return 1;
    }
    ...
    if (nerr > 0) {
        fprintf(stderr, "[minicc9] 检查出 %d 个错误，程序不会被执行\n", nerr);
        ast_free(prog);
        return 1;
    }
