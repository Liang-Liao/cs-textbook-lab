/* copied from lab09 wav_io, trimmed */
#include "wav_io.h"
#include <math.h>
#include <stdio.h>
#include <stdlib.h>
...
int wav_write(const char *path, const wav_data *w) {
    ...
    w16(f, w->channels);
    w32(f, w->sample_rate);
    w32(f, w->sample_rate * w->channels * (w->bits_per_sample / 8));
    w16(f, (uint16_t)(w->channels * (w->bits_per_sample / 8)));
    w16(f, w->bits_per_sample);
    fwrite("data", 1, 4, f);
    w32(f, db);
