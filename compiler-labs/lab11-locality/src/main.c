/*
 * main.c —— lab11 驱动：AST → 四元式 →【优化】→ 后端（四选一）
 *
 * 用法: ./minicc11.exe [-O] [选项] 文件.mc
 *   （缺省）生成 x86-64 汇编到 stdout；链接运行：
 *              ./minicc11.exe prog.mc > prog.s
 *              gcc prog.s src/rt.c -o prog.exe && ./prog.exe
 *   -O       在 gen 之后、后端之前跑优化器（常量折叠/传播、CSE、
 *            活跃变量死代码删除、窥孔、外提/展开/交换），统计摘要打
 *            stderr。不带 -O 行为与 lab8 一致——同一程序两种形态输出
 *            必须一致，这正是对拍的意义。
...
int main(int argc, char **argv)
{
    ...
    int want_cfg = 0, want_deps = 0, want_space = 0;
    ...
    if (!path) {
        fprintf(stderr,
            "用法: %s [-O] [-ir|-cfg|-deps|-space|-vm|-irvm|-eval|-trace|-ast|-symtab] 文件.mc\n",
            argv[0]);
        return 1;
    }
    ...
    if (nerr > 0) {
        fprintf(stderr, "[minicc11] 检查出 %d 个错误，程序不会被执行\n", nerr);
        ast_free(prog);
        return 1;
    }
