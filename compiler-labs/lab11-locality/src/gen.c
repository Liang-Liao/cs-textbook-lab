 ...
 *
 *   lab11 二维数组走**行主序扁平化降糖**：m[i][j] 在这里就展开成
 *   三条既有四元式 t=i*M; t2=t+j; v=m[t2]（M=列数）——四个执行引擎
 *   看到的仍是"一维数组按下标读写"，零改动；描述符登记的是总元素数
 *   N*M。降糖选在 gen 层而非 parser 层，是因为 eval 直接走 AST、
 *   要在树上做同样的行主序镜像（见 eval.c），而 -deps/-space 依赖
 *   分析则要**逆着**这条降糖把二维下标从四元式里恢复出来（opt.c）。
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
static const char *gen_expr(const AstNode *e)
{
    ...
    case A_INDEX: {
         ...
         * lab11 二维 m[i][j]：行主序降糖 t=i*M; t2=t+j; v=m[t2]，
         * 求值顺序与 eval 的镜像一致（先行后列，再合成下标）。 */
        const char *base = irname_typed(e->sym);
        const char *idx;
        if (e->c) {
            const char *row = gen_expr(e->a);
            const char *col = gen_expr(e->c);
            char mb[16];
            snprintf(mb, sizeof mb, "%d", e->sym->arr_dim2);
            const char *t1 = newtemp();
            note_type(t1, TY_INT);
            Quad *m1 = emit(Q_BINOP);
            m1->bop = T_STAR; m1->x = t1; m1->y = row; m1->z = str_dup(mb);
            const char *t2 = newtemp();
            note_type(t2, TY_INT);
            Quad *m2 = emit(Q_BINOP);
            m2->bop = T_PLUS; m2->x = t2; m2->y = t1; m2->z = col;
            idx = t2;
        } else {
            idx = gen_expr(e->a);
        }
...
static void gen_stmt(const AstNode *s, LoopCtx *ctx)
{
    ...
    case A_DECL:
        if (s->sym->arr_len > 0) {
             ...
             * lab11：二维按总元素数 N*M 登记扁平描述符。 */
            int total = s->sym->arr_len *
                        (s->sym->arr_dim2 > 0 ? s->sym->arr_dim2 : 1);
            note_array(irname_typed(s->sym), total,
    ...
    case A_STORE: {                   /* a[i] = e（6.4）：下标与右值都先求值
                                       * lab11 二维：先行后列合成扁平下标，
                                       * 再发一条 Q_STX——与 A_INDEX 同构 */
        const char *idx;
        if (s->c) {
            const char *row = gen_expr(s->a);
            const char *col = gen_expr(s->c);
            char mb[16];
            snprintf(mb, sizeof mb, "%d", s->sym->arr_dim2);
            const char *t1 = newtemp();
            note_type(t1, TY_INT);
            Quad *m1 = emit(Q_BINOP);
            m1->bop = T_STAR; m1->x = t1; m1->y = row; m1->z = str_dup(mb);
