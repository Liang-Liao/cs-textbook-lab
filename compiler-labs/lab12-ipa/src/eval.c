 ...
 *
 * lab11：二维数组在 AST 上仍是"一个数组 + 扁平 cells 存储"，但行主序
 * 地址合成在这里**镜像**一遍——flat = i*M + j（M=列数）。IR 路径的
 * 同一合成发生在 gen 层降糖（t=i*M; t2=t+j），两条路径必须逐字节
 * 一致，run_tests.sh 的四引擎对拍就是验收。求值顺序也保持一致：
 * 先行下标、后列下标，最后右值。
 */
#include <stdio.h>
#include <stdlib.h>
...
static Val eval_expr(const AstNode *e, Frame *env)
{
    ...
    case A_INDEX: {                      /* a[i]：沿帧链找数组，按下标取元素
        ...
        long flat = iv.i;
        if (e->c) {
            Val jv = eval_expr(e->c, env);
            flat = flat * e->sym->arr_dim2 + jv.i;
        }
        if (en && en->cells) v = en->cells[flat];   /* 越界不查（同 C） */
        break;
    }
    case A_NEG: {
...
static int eval_stmt(const AstNode *s, Frame *env, Val *ret)
{
    ...
    case A_DECL: {
        if (s->sym->arr_len > 0) {        /* 数组声明：开 len 格，零初始化
            ...
            frame_bind_array(env, s->sym, total);
            return EV_OK;
        }
        Val v = {0, 0, 0};
    ...
    case A_STORE: {                       /* a[i] = e：先下标后右值，顺序与 IR 一致
        ...
        long flat = iv.i;
        if (s->c) {
            Val jv = eval_expr(s->c, env);
            flat = flat * s->sym->arr_dim2 + jv.i;
        }
        Val rv = eval_expr(s->b, env);
        if (en && en->cells) en->cells[flat] = rv;  /* 越界不查（同 C） */
        return EV_OK;
    }
    case A_PRINT: {
