 ...
 *             （数组不作参数——没有指针退化，见下）
 *   decl    → (int|float) IDENT [ INT_LIT ] [ INT_LIT ] ;      ← 数组声明
 *             （lab11：第二维可选——int m[N][M] 声明二维数组，最多两维）
...
/* 解析一个下标组 [ expr ]：进入时 cur='['，返回时 cur 越过 ']'。
 * 行号经 *pline 带出（float 下标等报错定位用）。 */
static AstNode *parse_expr(void);       /* 前置：本文件后文有正式声明处 */
static AstNode *parse_one_subscript(int *pline)
{
    advance();                                 /* '[' */
    *pline = cur.line;
    AstNode *idx = check_valued(parse_expr(), *pline);
    match(T_RBRACKET);
    return check_index(idx, *pline);
}
...
static AstNode *parse_factor(void)
{
    ...
    if (cur.type == T_IDENT) {
        ...
        if (nxt.type == T_LBRACKET) {  /* ---- a[i] / m[i][j]：数组读 ---- */
            if (sym->arr_len == 0) {
                char msg[96];
            ...
            /* lab11：下标个数必须与声明维度一致（多一维少一维都报错，
             * 错误恢复时仍把已写出的下标组消耗干净，防错误扩散） */
            int want_dim = sym->arr_dim2 > 0 ? 2 : 1;
            advance();                 /* IDENT */
            int iline;
            AstNode *idx = parse_one_subscript(&iline);
            if (cur.type == T_LBRACKET) {
                AstNode *col = parse_one_subscript(&iline);
                if (want_dim == 1) {
                    char msg[96];
                    snprintf(msg, sizeof msg,
                             "一维数组 %s 只能带一个下标", name_tok.lexeme);
                    sem_error(&name_tok, msg);
                    return ast_err(line);
                }
                return ast_index2(sym, idx, col, line);
            }
            if (want_dim == 2) {
                char msg[96];
                snprintf(msg, sizeof msg, "二维数组 %s 需要两个下标",
                         name_tok.lexeme);
                sem_error(&name_tok, msg);
                return ast_err(line);
            }
...
static AstNode *parse_decl(void)
{
     ...
     * lab11 二维：int m[3][4];——第二维（列数）可选，最多两维 ---- */
    int arr_len = 0;
    int arr_dim2 = 0;
    if (cur.type == T_LBRACKET) {
        advance();
        ...
        match(T_RBRACKET);
        if (cur.type == T_LBRACKET) {          /* 第二维 */
            advance();
            if (cur.type == T_INT_LIT) {
                arr_dim2 = (int)cur.ival;
                advance();
                if (arr_dim2 <= 0)
                    sem_error(&name_tok, "数组第二维长度必须为正");
            } else {
                sem_error(&cur, "数组长度必须是整型字面量");
            }
            match(T_RBRACKET);
            /* 总元素数守卫：扁平化后是"一个大一维数组"，执行后端的
             * 全局区/帧容量按元素个数计——超限在编译期就拦下 */
            if (arr_dim2 > 0 && (long long)arr_len * arr_dim2 > 1048576)
                sem_error(&name_tok, "二维数组总元素数超过上限 1048576");
        }
    }
    ...
    Sym *sym = arr_len > 0
             ? sym_declare_array(name_tok.lexeme, t, arr_len, arr_dim2,
                                 name_tok.line)
...
/* a[i] = e ; / lab11：m[i][j] = e ; —— 数组元素赋值（6.4/lab11）。
 * dispatch 保证进入时 cur=IDENT、nxt='['。'=' 是强制的：`a[i];` 这种
 * 裸下标语句没有值效果，不合法，在 match(T_ASSIGN) 处报语法错误即可。
 * 下标个数与声明维度一致（一维一个、二维两个），不符报错防级联。 */
static AstNode *parse_store(int eat_semi)
{
    ...
    AstNode *col = NULL;
    int got2 = 0;
    if (cur.type == T_LBRACKET) {          /* 第二个下标（lab11） */
        col = parse_one_subscript(&iline);
        got2 = 1;
    }
    int want_dim = 0;
    if (sym && sym->kind == SYM_VAR && sym->arr_len > 0)
        want_dim = sym->arr_dim2 > 0 ? 2 : 1;
    if (want_dim == 1 && got2) {
        char msg[96];
        snprintf(msg, sizeof msg, "一维数组 %s 只能带一个下标",
                 name_tok.lexeme);
        sem_error(&name_tok, msg);
    } else if (want_dim == 2 && !got2) {
        char msg[96];
        snprintf(msg, sizeof msg, "二维数组 %s 需要两个下标",
                 name_tok.lexeme);
        sem_error(&name_tok, msg);
    ...
    return got2 ? ast_store2(sym, idx, col, rhs, name_tok.line)
                : ast_store(sym, idx, rhs, name_tok.line);
}
