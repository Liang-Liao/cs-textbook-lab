/*
 * lexer.c —— lab6 用的词法分析器（lab5 的原样复制；lab4 又是 lab1 的
 * 精简复刻：去掉符号表驻留，词素直接 strdup，其余行为与 lab1 完全一致，
 * 详细注释见 lab1）。放在这里是为了让 lab6 完全自包含、可独立阅读。
 *
 * lab6 升级的是 AST 之后的翻译（三地址码生成/回填/过程调用），词法层
 * 与 lab3/lab4/lab5 逐字节一致——`&& || ! , ( ) return void` 这些
 * lab6 要用的记号，lab1 造词法器时就全部预留好了。
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
const char *token_name(TokenType t)
{
    static const char *NAMES[] = {
        ...
        "T_KW_IF", "T_KW_ELSE", "T_KW_WHILE", "T_KW_FOR", "T_KW_RETURN",
