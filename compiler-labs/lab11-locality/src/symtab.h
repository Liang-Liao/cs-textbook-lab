...
typedef struct Sym {
    ...
    int         arr_len;   /* >0 = 数组（第一维/行数，编译期常量）；
                            * 0 = 标量（6.4 节数组翻译的静态信息来源） */
    int         arr_dim2;  /* lab11：第二维/列数；0 = 一维数组或标量。
                            * 二维数组的总元素数 = arr_len * arr_dim2，
                            * gen 按"行主序扁平化"登记进 IR 描述符 */
    int         line;      /* 声明处行号 */
...
/* 登记数组（6.4 + lab11 二维）：type 存元素类型，len = 第一维（行数），
 * dim2 = 第二维（列数；一维传 0）。长度必须是编译期常量（parser 保证），
 * MiniC 不做指针退化——数组不能作函数参数/返回值，只能整体声明后
 * 按下标使用。二维数组的行主序降糖发生在 gen 层（见 gen.c），符号表
 * 只负责记全两维的静态信息。 */
Sym *sym_declare_array(const char *name, Type elem, int len, int dim2,
                       int line);
