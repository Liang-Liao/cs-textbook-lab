/*
 * main.c —— lab12 驱动：AST → 四元式 →【过程间分析+优化】→ 后端（四选一）
 *
 * 用法: ./minicc12.exe [-O] [选项] 文件.mc
 *   （缺省）生成 x86-64 汇编到 stdout；链接运行：
 *              ./minicc12.exe prog.mc > prog.s
 *              gcc prog.s src/rt.c -o prog.exe && ./prog.exe
 *   -O       在 gen 之后、后端之前跑优化器（过程间分析：IPCP 常量
 *            代入与函数内联；常量折叠/传播、CSE、活跃变量死代码删除、
 *            窥孔、外提/展开/交换），统计摘要打 stderr。不带 -O 行为
 *            与 lab8 一致——同一程序两种形态输出必须一致，这正是
 *            对拍的意义。
 *   -deps    依赖分析观测（lab11）：对每个自然循环列出访存语句对的
 *            距离向量与类别，打 stderr。固定作用于 gen 后、-O 前的
 *            原始 IR——分析的是源程序本来的循环结构
 *   -space   迭代空间可视化（lab11）：完美二重嵌套画 ASCII 网格 +
 *            依赖箭头样例；流约定同 -deps
 *   -cg      调用图转储（lab12）：结点=函数段，边=CALL 四元式，
 *            叶子/递归环就地标注。流约定同 -cfg/-deps——看的是
 *            gen 后、-O 前的原始调用结构
...
int main(int argc, char **argv)
{
    ...
    int want_cfg = 0, want_deps = 0, want_space = 0, want_cg = 0;
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (strcmp(argv[i], "-deps") == 0)   want_deps = 1;
        else if (strcmp(argv[i], "-space") == 0)  want_space = 1;
        else if (strcmp(argv[i], "-cg") == 0)     want_cg = 1;
    ...
    if (!path) {
        fprintf(stderr,
            "用法: %s [-O] [-ir|-cg|-cfg|-deps|-space|-vm|-irvm|-eval|-trace|-ast|-symtab] 文件.mc\n",
            argv[0]);
        return 1;
    }
    ...
    if (nerr > 0) {
        fprintf(stderr, "[minicc12] 检查出 %d 个错误，程序不会被执行\n", nerr);
        ast_free(prog);
        return 1;
    }
    ...
    /* lab11 观测点固定在 -O 之前：依赖分析看的是源程序本来的循环，
     * 而不是被 LICM/展开/交换改写后的形状（-O 的统计摘要在 stderr） */
    if (want_deps)  { deps_dump(irp);  ast_free(prog); return 0; }
    if (want_space) { space_dump(irp); ast_free(prog); return 0; }

    /* lab12 观测点：调用图同样看 gen 后、-O 前的原始结构 */
    if (want_cg) {
        cg_dump(irp);
        ast_free(prog);
        return 0;
    ...
    if (want_O)
        opt_run(irp);                     /* lab9 折叠/CSE/DCE + lab10 循环
                                           * 优化 + lab11 门禁展开与交换 +
                                           * lab12 过程间分析（IPCP/内联） */
