#!/usr/bin/env bash
# lab11 测试：二维数组四引擎对拍 + 依赖分析/迭代空间快照 + 合法性驱动的
#             变换（门禁展开/循环交换正反例）+ lab10 全量回归（期望零改动）
# 用法: bash tests/run_tests.sh   （在 lab11-locality 目录下执行）
...
if ! gcc -std=c11 -Wall -Wextra -O2 -o "$BIN" $SRCS 2> /tmp/lab11_cc.log; then
    echo "[FAIL] 编译失败"; cat /tmp/lab11_cc.log; exit 1
fi
if [ -s /tmp/lab11_cc.log ]; then
    echo "[FAIL] 编译有警告:"; cat /tmp/lab11_cc.log; exit 1
fi

gcc -std=c11 -O2 -c src/rt.c -o "$TMP/rt.o" || { echo "[FAIL] rt.c 编译失败"; exit 1; }

check() { # check <名字> <实际文件> <期望文件>
    if diff -u <(tr -d '\r' < "$3") <(tr -d '\r' < "$2") > /tmp/lab11_diff.log 2>&1; then
        echo "[PASS] $1"; pass=$((pass+1))
    else
        echo "[FAIL] $1"; head -20 /tmp/lab11_diff.log; fail=$((fail+1))
    fi
}
...
# 1) 旧用例 + lab11 新用例 + 审计修复回归：带 -O 的机器码输出与期望逐字节
#    一致。unrollinit 钉住 for 赋值式 init 解析与展开初值门禁；foldcmp 钉
#    折叠整型比较的 long 口径；swapacc/swapcall/swapinit 是交换三门禁的
#    负例（输出即正确语义，拒绝证据由 §2c 的统计断言给出）
for c in expr vars bool func ir arr vm collide lics unroll licsunroll \
         mat dep swap swapbad space unrollnew unrolldenied \
         unrollinit foldcmp swapacc swapcall swapinit; do
    if native_O "$c"; then
...
# 2c) 审计修复回归：交换三门禁负例——标量携带依赖(swapacc)/体含 CALL
#     (swapcall)/空 init 起点未知(swapinit) 都必须拒绝，stderr 统计里
#     交换计数为 0；语义正确性由 §1 的输出对拍钉住
...
if [ $allok -eq 1 ]; then
    echo "[PASS] 四引擎×{-O}对拍(x86-64 vs vm vs irvm vs eval, 22 个程序)"; pass=$((pass+1))
else
...
echo "----------------------------------------"
echo "lab11: $pass passed, $fail failed"
[ "$fail" -eq 0 ]
